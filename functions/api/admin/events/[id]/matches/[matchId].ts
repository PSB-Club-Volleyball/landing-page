...
export const onRequestPatch: PagesFunction<Env, 'id' | 'matchId', AdminData> = async ({ request, env, params, data }) => {
  ...
  const match = await env.DB.prepare(
    `SELECT m.id, m.team_a_id, m.team_b_id, e.format_config
     FROM event_matches m JOIN events e ON e.id = m.event_id
     WHERE m.id = ?1 AND m.event_id = ?2`
  )
    .bind(matchId, eventId)
    .first<{ id: number; team_a_id: number | null; team_b_id: number | null; format_config: string | null }>()
  ...
  await env.DB.prepare(
    `UPDATE event_matches SET scores = ?1, forfeit_team_id = ?2, winner_id = ?3 WHERE id = ?4`
  )
    .bind(
