...
// PUT /api/admin/roster/:id -> partial update, any subset of FIELDS
export const onRequestPut: PagesFunction<Env, 'id', AdminData> = async ({ request, env, params, data }) => {
  ...
  const result = await env.DB.prepare(
    `UPDATE roster SET ${setClause}, updated_at = CURRENT_TIMESTAMP WHERE id = ?${updates.length + 1}`
  )
    .bind(...values, id)
    .run()

  if (result.meta.changes === 0) return notFound('Player not found')

  await logAudit(env, data.user.id, 'update', 'roster', id, body)
  return json({ ok: true })
}

// DELETE /api/admin/roster/:id -> owner only
export const onRequestDelete: PagesFunction<Env, 'id', AdminData> = async ({ env, params, data }) => {
  ...
  const result = await env.DB.prepare(`DELETE FROM roster WHERE id = ?1`).bind(id).run()
  if (result.meta.changes === 0) return notFound('Player not found')

  await logAudit(env, data.user.id, 'delete', 'roster', id)
  return json({ ok: true })
}
