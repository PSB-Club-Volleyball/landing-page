...
export async function getLoginSettings(env: Env): Promise<LoginSettings> {
  const row = await env.DB.prepare(
    `SELECT google_enabled, microsoft_enabled, microsoft_other_enabled, current_season, roster_visible FROM login_settings WHERE id = 1`
  ).first<{
    google_enabled: number
    microsoft_enabled: number
    microsoft_other_enabled: number
    current_season: string | null
  ...
  return {
    ...
    microsoft_other_enabled: row.microsoft_other_enabled === 1,
...
export function isProviderEnabled(name: string, settings: LoginSettings): boolean {
  ...
  if (name === 'microsoft-other') return settings.microsoft_other_enabled
  return false
}
